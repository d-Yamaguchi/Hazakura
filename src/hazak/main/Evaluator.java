...
public class Evaluator extends CalcVisitor {
	...
	public Object visit(NameBind node) {
		String id = (String)node.child.get(0).accept(this);
		if (!record.containsKey(id))
			record.put(id, node.child.get(1));//Bindをレコードに保存する
		else {
	...
	public Object visit(Func node) {
		...
		recordPrime.put((String) node.child.get(0).accept(this), argument.pop());
		CalcTree rightNode = node.child.get(1);
		CalcTree rightNode_cnode = rightNode.child.get(rightNode.child.size()-1);
		if (rightNode_cnode instanceof Func) {
			return rightNode;//子ノードにFunc型が含まれる場合
		}else {
			//funcが含まれないときここから
			Object returnVal = node.child.get(1).accept(this);
			checkSum--;
			if (checkSum == 0)
				recordPrime.clear();
			return returnVal;
	...
	public Object visit(Apply node) {
		argument.push(node.child.get(1).accept(this));//右ノード引数を取得(Eval)してArg置き場にセットしておく
		CalcTree leftNode = node.child.get(0);
		...
		if (leftNode instanceof Name) {
			...
			CalcTree func = (CalcTree) leftNode.accept(this);//左ノードを評価するとFunc型のtreeが戻ってくるはずです
	...
	public Object visit(And node) {
		Boolean left = (Boolean)node.child.get(0).accept(this);
		Boolean right = (Boolean)node.child.get(1).accept(this);
		return left && right;
	}
	@Override
	...
	public Object visit(Equals node) {
		return node.child.get(0).accept(this) == node.child.get(1).accept(this);
	}
	@Override
	...
	public Object visit(GreaterThan node) {
		Integer left = (Integer)node.child.get(0).accept(this);
		Integer right = (Integer)node.child.get(1).accept(this);
		return left > right;
	}
	@Override
	...
	public Object visit(Minus node) {
		return -1 * (Integer)node.child.get(0).accept(this);
	}
	@Override
	public Object visit(Conditional node) {
		for (int i = 0; i < node.child.size(); i++) {
			Object cvalue = node.child.get(i).accept(this);
	...
	public Object visit(Truereturn node) {
		if ((boolean) node.child.get(1).accept(this)) {
